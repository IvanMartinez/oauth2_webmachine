...
-define(PATH, "/authorization_token").
...
bad_request_tests(_Config)->
    Request2 = test_util:make_get_wrq(?PATH, 
                                      [{"response_type", "token"},
                                       ...
                                       {"scope", "root.b.c"}], []),
    {Result2, _Response2, Context2} = authorization_token:process_get(Request2, 
                                                                  ?CONTEXT),
...
invalid_redirection_uri_tests(_Config)->
    ...
    {Result1, _Response1, Context1} = authorization_token:process_get(Request1, 
                                                                  ?CONTEXT),
...
invalid_request_tests(_Config)->
    Request1 = test_util:make_get_wrq(?PATH, 
                                      [{"client_id", ?CLIENT1_ID},
                                       {"redirect_uri", ?CLIENT1_URI}],
                                      []),
    {Result1, Response1, Context1} = authorization_token:process_get(Request1, 
                                                                  ?CONTEXT),
    Request2 = test_util:make_get_wrq(?PATH, 
                                      [{"client_id", ?CLIENT1_ID},
                                       {"redirect_uri", ?CLIENT1_URI},
                                       {"scope", "root.a root.b.*"},
                                       {"state", ?STATE}], []),
    {Result2, Response2, Context2} = authorization_token:process_get(Request2, 
                                                                  ?CONTEXT),
    [?_assertEqual({halt, 302}, Result1),
     ?_assertEqual(?CLIENT1_URI ++"?error=invalid_request", 
                   wrq:get_resp_header("Location", Response1)),
     ?_assertEqual(?CONTEXT, Context1),
     ?_assertEqual({halt, 302}, Result2),
     ?_assertEqual(?CLIENT1_URI ++"?error=invalid_request&state=" ++ ?STATE, 
...
unsupported_response_type_tests(_Config)->
    ...
    {Result2, Response2, Context2} = authorization_token:process_get(Request2, 
                                                                  ?CONTEXT),
    [?_assertEqual({halt, 302}, Result1),
     ?_assertEqual(?CLIENT1_URI ++"?error=unsupported_response_type", 
                   wrq:get_resp_header("Location", Response1)),
     ?_assertEqual(?CONTEXT, Context1),
     ?_assertEqual({halt, 302}, Result2),
     ?_assertEqual(?CLIENT1_URI ++"?error=unsupported_response_type&state=" ++
                       ?STATE, 
                   wrq:get_resp_header("Location", Response2)),
     ?_assertEqual(?CONTEXT, Context2)
     ].
...
successful_tests(_Config)->
    ...
    {Result1, _Response1, Context1}  = authorization_token:process_get(Request1, 
                                                                   ?CONTEXT),
    {ok, StoredRequest1} = oauth2_ets_backend:retrieve_request(
                             <<?REQUEST1_ID>>),
    ...
    Request3 = test_util:make_get_wrq(?PATH, 
                                      [{"response_type", "token"},
                                       ...
                                       {"state", ?STATE}],
                                      []),
    {Result3, _Response3, Context3}  = authorization_token:process_get(Request3, 
                                                                   ?CONTEXT),
    {ok, StoredRequest3} = oauth2_ets_backend:retrieve_request(
                             <<?REQUEST2_ID>>),
    [?_assertEqual({halt, 200}, Result1),
     ?_assertEqual(?CONTEXT, Context1),
     ?_assertEqual(#oauth2_request{client_id = <<?CLIENT1_ID>>, 
                                   redirect_uri = <<?CLIENT1_URI>>,
